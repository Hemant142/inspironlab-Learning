...
import type { CartsItem, ProductData, PurchaseItem } from "../../../TypeScript/constrain";
import { addToCart, deleteCarts, fetchProducts, fetchSingleProduct, getCarts, getMyOrder, myOrderPlaced, updateQuantityCarts } from "./productThunks";

const initialState:{products:Array<ProductData>,singleProduct:ProductData|object, productLength:number, orderedProducts:Array<PurchaseItem>, carts:Array<CartsItem>}={
    products: [],
    singleProduct:{},
    productLength: 0,
    orderedProducts: [],
...
const productSlice = createSlice({
    ...
    initialState,
    reducers:{
        getProducts:(state,action: PayloadAction<{ products:Array<ProductData>, productLength:number }> )=>{
        ...
        orderProducts:(state,action:PayloadAction<{orderedProducts:Array<PurchaseItem>}>)=>{
            const {orderedProducts}=action.payload;
            state.orderedProducts=orderedProducts;
        },
        cartsProducts:(state,action:PayloadAction<{cartProducts:Array<CartsItem>}>)=>{
    ...
    extraReducers:(builder)=>{
        ...
        .addCase(deleteCarts.fulfilled,(state,action)=>{
         state.carts = state.carts.filter(item => item.product._id !== action.meta.arg.id)
